...
export default function MapTilerComponent({ gymsArray }) {
  ...
  useEffect(() => {
    ...
    gymsArray.forEach((gym) => {
      ...
      btn.style.cssText =
        "background:#007bff; color:white; border:none; padding:5px 10px; cursor:pointer; margin-top:5px;";
      ...
      if (profile?.role === "admin") {
        ...
        btn.addEventListener("click", (e) => {
          ...
          window.location.href = `/admin/AdminGymDetails?q=${encodeURIComponent(gym.title)}`;
        });
        ...
        btn.addEventListener("click", (e) => {
          e.stopPropagation(); // ¡Vital para que el mapa no ignore el clic en Android!
          window.location.href = `/user/my-bookings?name=${encodeURIComponent(gym.title)}&id=${encodeURIComponent(gym.id)}`;
        });
      } else {
        //   htmlContent = `
        //   <div style="text-align:center; background:gray;">
