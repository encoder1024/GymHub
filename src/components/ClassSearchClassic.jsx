...
export default function ClassSearch() {
  ...
  // columnas donde buscar en classes_santa_fe
  ...
  return (
    <div className="container">
      <h2>Buscar Clases en Santa Fe</h2>

      <input
        type="text"
        placeholder="Buscar por nombre o descripción de la clase..."
        value={query}
          ...
          <tbody>
            {results.map((cls) => (
              <tr key={cls.id}>
                ...
                {/* gym info */}
                <td dangerouslySetInnerHTML={{ __html: highlight(cls.gymsSantaFe?.title) }} />
                <td dangerouslySetInnerHTML={{ __html: highlight(cls.gymsSantaFe?.street) }} />
                ...
                <td style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  {cls.gymsSantaFe?.phone && (
                    <button
                      onClick={() => {
                        const formatted = formatPhoneForWhatsApp(cls.gymsSantaFe.phone);
                        if (formatted) {
